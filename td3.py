...
class TD3(object):
    ...
    def __init__(self, sess, state_dim, action_dim, max_action, min_action, actor_learning_rate, critic_learning_rate, tau, RANDOM_SEED, device = '/cpu:0'):
        ...
        with tf.variable_scope('learning_rate'): 
            ...
            self.actor_decay_learning_rate = tf.train.exponential_decay(self.actor_learning_rate, self.global_step, 400000, 0.96, staircase=True)
            self.critic_decay_learning_rate = tf.train.exponential_decay(self.critic_learning_rate, self.global_step, 400000, 0.96, staircase=True)
    ...
    def _build_net(self,scope):
        ...
        with tf.device(self.device):        
            ...
            with tf.variable_scope(scope + '/actor'):
                ...
                a = tf.contrib.layers.fully_connected(l2, self.a_dim, weights_initializer=w_init, activation_fn=tf.nn.tanh) # None  tf.nn.tanh
                scaled_a = tf.multiply(a, self.max_action)
    ...
    def train(self, s_batch, a_batch, r_batch, t_batch, s2_batch, MINIBATCH_SIZE):
        ...
        if self.count == 2:
            ...
            noise = np.clip(0.1*np.random.randn(MINIBATCH_SIZE,self.a_dim) + 0.,-0.5,0.5)
            a_outs = np.clip(a_outs + noise, self.min_action,self.max_action)
    ...
    def save(self, num = '0'):
        num = str(num)
        #saving = './model/model' + num + '.ckpt'
        self.saver.save(self.sess,'./model/model' + num + '.ckpt')
        self.saver_target.save(self.sess,'./model/model_target' + num + '.ckpt')
    ...
    def load(self, num ='0'):
        print('loading model ************')
        num = str(num)
        self.saver.restore(self.sess,'./model/model' + num + '.ckpt')
        self.saver_target.restore(self.sess,'./model/model_target' + num + '.ckpt')
